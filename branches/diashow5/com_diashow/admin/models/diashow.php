...
class DiashowModelDiashow extends JModelAdmin{
	...
	public function getScript()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_diashow.edit.diashows.data', array());
		if (empty($data)){
			$data = $this->getItem();
		}
		return $data;
        ...
        protected function allowEdit($data = array(), $key = 'id'){
                ...
                return JFactory::getUser()->authorise('core.edit', 'com_diashow.diashow.'.((int) isset($data[$key]) ? $data[$key] : 0)) or parent::allowEdit($data, $key);
        }
	...
	public function getMenu(){
		...
		$query ="SELECT id , alias , menutype FROM #__menu where published = '1' order by  menutype, id";
